 ...
 */
class GetHistoryBooks @Inject constructor(executor: ThreadPoolExecutor, var repository: HistoryBooksRepository, var userRepository: UserRepository) : UseCase<List<HistoryBook>, Void?>(executor) {
    override fun process(unused: Void?, resultListener: OnResultListener<List<HistoryBook>>, errorListener: OnErrorListener) {
        try {
            userRepository.get{
                    repository.get(it.cgu, resultListener)
                }
        } catch (e: Exception) {
            errorListener(e)
        }
