...
public class ReloadHitoryBooks extends UseCase<List<HistoryBook>,Void > {
    HistoryBooksRepository repository;
    UserRepository userRepository;

    @Inject
    public ReloadHitoryBooks(ThreadPoolExecutor executor, HistoryBooksRepository repository,UserRepository userRepository) {
        super(executor);
    ...
    protected void process(Void unused, OnResultListener<List<HistoryBook>> resultListener, OnErrorListener errorListener) {
        try{
            userRepository.get(
                    user->repository.reload(user.getCgu(),resultListener)
            );
        }catch (Exception e){
