...
public class GetLoanBooks extends UseCase<List<LoanBook>,Void > {
    LoanBooksRepository booksRepository;
    UserRepository userRepository;
    @Inject
    public GetLoanBooks(ThreadPoolExecutor executor, LoanBooksRepository booksRepository, UserRepository userRepository) {
        super(executor);
    ...
    @Override
    protected void process(Void unused, OnResultListener<List<LoanBook>> resultListener, OnErrorListener errorListener) {
        try{
            userRepository.get((user)->
                booksRepository.get(user.getCgu(),resultListener)
            );
        }catch (Exception e){
