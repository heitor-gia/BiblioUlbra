 ...
 */
class ReloadLoanBooks @Inject constructor(executor: ThreadPoolExecutor, var booksRepository: LoanBooksRepository, var userRepository: UserRepository) : UseCase<List<LoanBook>, Void?>(executor) {
    public override fun process(unused: Void?, resultListener: OnResultListener<List<LoanBook>>, errorListener: OnErrorListener) {
        try {
            userRepository.get {
                booksRepository.reload(it.cgu, resultListener)
            }
        } catch (e: Exception) {
            errorListener(e)
        }
